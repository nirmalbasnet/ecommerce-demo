            ...
            <div class="table-responsive">
                ...
                <table class="table m-0 table-bordered common-table">
                    ...
                    @if(isset($inquiries) && $inquiries->count() > 0)
                        ...
                        @foreach($inquiries as $datum)
                            @php $sn ++; @endphp
                            <tr id="{{$datum->id}}">
                                <td>{{$sn}}</td>
                                <td>{{$datum->name}}</td>
                                <td>{{$datum->email}}</td>
                                <td><p style="text-align: justify;">{{$datum->message}}</p></td>
                                <td class="action-td">
                                    <a href="#" class="table-content-delete delete-inquiry"
                        ...
                        @endforeach
                        <tr>
                            <td colspan="5">{{$inquiries->links()}}</td>
                        </tr>
                    @else
                        <tr>
    ...
    <script>
        $('.delete-inquiry').on('click', function (e) {
            ...
            alertify.confirm("Are you sure ?",
                function () {
                    $.ajax({
                        url: baseurl + '/admin/inquiries/'+id+'/delete',
                        type: 'get',
